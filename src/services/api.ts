...
const API_KEY1="52b81c52accbbeb226ab3f96d59f144c";
export const buscarNoticias = async () => {
  try {
    const response = await fetch(
      `https://gnews.io/api/v4/search?q=plantas medicinais&lang=pt-br&max=10&apikey=${API_KEY1}`
    );
...
export const buscarInformacoesPlantas = async (nomePlanta: string) => {
  try {
    const response = await fetch(
      `https://gnews.io/api/v4/search?q=${encodeURIComponent(nomePlanta)}&lang=pt-br&max=5&apikey=${API_KEY1}`
    );
    const data = await response.json();
    console.log('RESPOSTA DA API:', data);
    return data.articles || [];
